...
model {
  ...
  for(i in 1:n){
    m_log[i] = mbar + spp_effects[spp_id_num[i],1];

    p_j[i] = pbar + p_plot[plot_id_num[i]]*sd_plot  + spp_effects[spp_id_num[i], 2];
...
generated quantities{
  ...
  for(i in 1:n){
    m[i] = exp(mbar + spp_effects[spp_id_num[i],1]);

    p[i] = exp(pbar + p_plot[plot_id_num[i]]*sd_plot  + spp_effects[spp_id_num[i], 2]);
