...
sp_max_log <- rnorm(12, log(60), sd = .2)
...
sp_curves <- expand_grid(x_vals = runif(50, -6, 12),
            sp = 1:12) |> 
  mutate(y_mean = sp_max_log[sp] + 
           plogis(sp_low_prop_logit, log.p = TRUE) + 
           log1p(exp(-sp_low_prop_logit) * plogis(exp(sp_a[sp])*(x_vals - b))
             ),
         y_meas = rlnorm(length(x_vals), ln_a(y_mean, .5), ln_b(y_mean, .5))
  )
